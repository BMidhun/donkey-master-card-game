...
function PlayerDeckComponent({playerCards}:IProps) {
  let top=0;
  return (
    <div className={`my-2 flex items-baseline justify-between grow overflow-auto border-2 shadow shadow-grey-400 ${getPlayerColor(PLAYERS_ENUM.HUMAN)}`}>
        {Object.keys(playerCards).map(item => {
             ...
             return <div key={item} className="h-full w-full relative">
                        ...
                        playerCards[k].map((card, index) => {
                            top = index * 8
                            return  <div key={card.rank} className="cards" style={{position:"absolute", top:`${top}%`, transform:"scale(0.75)"}}>
