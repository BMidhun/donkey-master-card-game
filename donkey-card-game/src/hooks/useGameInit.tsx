...
function useGameInit() {
  ...
  const [currentPlayerTracker, setCurrentPlayerTracker] = useState(-2);
  ...
  useEffect(() => {
    if (isPlayerReady) {
        setScreenText({ type: "ROUND", message: "READY TO PLAY!!" });
      setTimeout(() => {
        setScreenText(null);
        setCurrentPlayerTracker(0);
      },1000);
    } else setScreenText({ type: "ROUND", message: "Shuffling cards!!" });
  }, [isPlayerReady]);
  ...
  const changePlayOrderTracker = useCallback(
    function (player: PLAYERS_ENUM | false) {
      if (player === false) {
  ...
  function popPlayer(winners: PLAYERS_ENUM[]) {
    setGameState((prev) => {
      ...
      return prev.numOfAvailablePlayers === NUM_OF_PLAYERS - winners.length
        ? prev
        : {
  ...
  function removeCardOnDeal(player: PLAYERS_ENUM, card: ICard) {
    setPlayerState((prev) => ({
      ...
      [player]: {
        ...prev[player],
        [card.type]: prev[player][card.type].filter(
          (_card) => _card.value !== card.value
        ),
      },
    }));
  }
  ...
  const checkWinner = useCallback(
    function () {
      ...
      for (let player of Object.keys(state)) {
        ...
        if (
          state[key][CARD_TYPE_ENUM.CLUBS].length === 0 &&
      ...
      if (winners.length) popPlayer(winners);
    },
    [playerState]
  );
